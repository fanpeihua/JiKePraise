...
public class CountView extends View {
    ...
    public CountView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mTextSize = typedArray.getDimension(R.styleable.CountView_cv_text_size, ThumbUtils.sp2px(context, 15f));
        typedArray.recycle();
        init();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setMeasuredDimension(ThumbUtils.getDefaultSize(widthMeasureSpec,
                getContentWidth() + getPaddingLeft()),
    ...
    private void calculateLocation() {
        ...
        float unChangedWidth = textWidth * mTexts[0].length();

        Paint.FontMetricsInt fontMetrics = mTextPaint.getFontMetricsInt();
        float y = getPaddingTop() + (getContentHeight() - fontMetrics.bottom - fontMetrics.top) / 2;

        mTextPoints[0].x = getPaddingLeft();
        mTextPoints[1].x = getPaddingLeft() + unChangedWidth;
        mTextPoints[2].x = getPaddingLeft() + unChangedWidth;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mTextPaint.setColor((Integer) ThumbUtils.evaluate(mFraction, mEndTextColor, mTextColor));
        canvas.drawText(String.valueOf(mTexts[1]), mTextPoints[1].x, mTextPoints[1].y, mTextPaint);

        mTextPaint.setColor((Integer) ThumbUtils.evaluate(mFraction, mTextColor, mEndTextColor));
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        mTextSize = data.getFloat("textSize", ThumbUtils.sp2px(getContext(), DEFAULT_TEXT_SIZE));
